...
public class IntegrationTests(AppFixture fixture) : IClassFixture<AppFixture>
{
    ...
    /// Намеренно используется повторяющийся id (1001), чтобы убедиться,
    ...
    /// <summary>
    /// Ожидает появления файла в S3 с заданным id (поллинг с таймаутом).
    /// Необходим из-за асинхронной природы SNS → FileService → S3.
    /// </summary>
